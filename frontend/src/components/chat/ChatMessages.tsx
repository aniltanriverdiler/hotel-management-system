...
export default function ChatMessages({ 
  ...
  const renderMessage = (message: SocketMessage) => {
    const isOwnMessage = currentUser && message.sender_id === currentUser.user_id;
    const { initial, colorClass } = getUserAvatar(message.sender);
    
    return (
      <div key={message.message_id} className={`flex items-start gap-3 ${isOwnMessage ? 'justify-end' : ''}`}>
        {/* Avatar - Sol tarafta (diğer kullanıcılar için) */}
        {!isOwnMessage && (
        ...
        <div className={`rounded-lg p-4 max-w-[75%] ${
          ...
          {/* Sender name (sadece diğer kullanıcılar için) */}
          {!isOwnMessage && (
        ...
        {/* Avatar - Sağ tarafta (kendi mesajlarımız için) */}
        {isOwnMessage && (
          <div className={`w-8 h-8 rounded-full ${colorClass} flex items-center justify-center flex-shrink-0`}>
            <span className="text-white text-sm font-medium">{initial}</span>
          </div>
    ...
    <CardContent className="flex-1 p-6 overflow-y-auto text-base space-y-4">
      ...
      {isLoading && (
        <div className="flex items-center justify-center py-8">
          ...
          <span className="ml-2 text-muted-foreground">Mesajlar yükleniyor...</span>
        </div>
      )}
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="bg-muted rounded-lg p-4 max-w-[75%]">
            <p className="text-base leading-relaxed">
              Merhaba! Size nasıl yardımcı olabilirim? Otel rezervasyonu, hizmetler veya diğer konularda sorularınızı sorabilirsiniz.
            </p>
          </div>
        </div>
      )}
